...
// Helper function to compute kernel volume
static int compute_volume_helper(
  ...
  if (region_type == RegionType::HYPER_CUBE) {
    ...
    // Sum of (k-1) + 1 for all dimensions
    int volume = 1;
    for (auto k : kernel_size) {
      TORCH_CHECK(k > 0, "kernel_size must be positive");
...
KernelGenerator::KernelGenerator(
    ...
    : kernel_size_(kernel_size.begin(), kernel_size.end()),
      kernel_stride_(stride.begin(), stride.end()),
      kernel_dilation_(dilation.begin(), dilation.end()),
...
void KernelGenerator::initialize() {
  ...
  // Expand scalar kernel_size, stride, dilation to vectors
  ...
  // Validate region_type specific requirements
  ...
  // Compute requires_strided_coordinates
...
KernelGenerator::KernelParams KernelGenerator::get_kernel(
  ...
  // Determine up_stride for transpose convolutions
  stride_type up_stride(D_, 1);
  if (is_transpose) {
    up_stride = kernel_stride_;
  }
  
  // Convert region type (handles HYBRID -> CUSTOM conversion)
