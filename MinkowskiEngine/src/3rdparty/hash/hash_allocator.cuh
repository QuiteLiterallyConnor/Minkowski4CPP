...
struct managed_allocator {
  ...
  T* allocate(std::size_t n, cudaStream_t stream = 0) const
  {
    ...
    if (error != cudaSuccess) {
      ...
      throw std::runtime_error("cudaMalloc failed in the hash_allocator.cuh:managed_allocator.");
    }
    return d_tmp;
...
struct default_allocator {
  ...
  T* allocate(std::size_t n, cudaStream_t stream = 0) const
  {
    ...
    if (error != cudaSuccess) {
      cudaGetLastError();  // clear CUDA error
      throw std::runtime_error("cudaMalloc failed in the hash_allocator.cuh:default_allocator.");
    }
